...
function chatPublic(row: typeof userAiConfigs.$inferSelect): PublicModelConfig {
  return {
    ...
    runtimePresetId: chatRuntimeId(row.id),
    connectionPresetId: row.connectionPresetId ?? undefined,
...
function imagePublic(row: typeof userImageConfigs.$inferSelect): PublicModelConfig {
  return {
    ...
    apiKeyConfigured: true,
    apiKeyLast4: row.apiKeyLast4,
    runtimePresetId: imageRuntimeId(row.id),
    createdAt: row.createdAt,
    updatedAt: row.updatedAt,
  };
}
...
export async function createModelConfig(userId: string, input: z.infer<typeof modelConfigCreateSchema>) {
  if (input.kind === "chat") {
    ...
    const row = {
      id: randomUUID(),
      userId,
      connectionPresetId: resolved.presetId,
      name: input.name,
      provider: resolved.provider,
      baseUrl: resolved.baseUrl ?? "",
      model: resolved.model,
      ...encrypted,
      apiKeyLast4: resolved.apiKey!.slice(-4),
    };
...
export async function updateModelConfig(userId: string, id: string, input: z.infer<typeof modelConfigUpdateSchema>) {
  if (input.kind === "chat") {
    ...
    const values = {
      ...
      apiKeyLast4: input.apiKey ? input.apiKey.slice(-4) : current.apiKeyLast4,
    };
...
export async function deleteModelConfig(userId: string, id: string) {
  ...
  if (chat[0]) {
    const config = chatPublic(chat[0]);
    await getDb().delete(userAiConfigs).where(eq(userAiConfigs.id, id));
    await auditModelConfig({ userId, configId: id, kind: "chat", action: "deleted", outcome: "success", provider: config.provider, model: config.model, runtimePresetId: config.runtimePresetId });
    return;
  }
...
export async function imageConnectionForPreset(userId: string, presetId: string): Promise<{ connection: ImageConnection; config: PublicModelConfig } | null> {
  ...
  const rotated = rotatedImageCredentials(row);
  if (rotated) await getDb().update(userImageConfigs).set(rotated).where(eq(userImageConfigs.id, row.id));
  return { connection, config: imagePublic(row) };
}

export function imagePreset(config: PublicModelConfig) {
  return { id: config.runtimePresetId, label: config.name, model: config.model, ...imageCapabilities(config.provider as ImageProvider) };
}
