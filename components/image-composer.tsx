...
export function ImageComposer({ presets, presetId, draft, references, isGenerating, isUploading, onPresetChange, onDraftChange, onUpload, onRemove, onGenerate, onStop, onOpenConfig }: {
  ...
  return <section className="image-composer-panel" aria-label="生图参数">
    ...
    <button className="chat-primary-button image-generate-button" type="button" onClick={isGenerating ? onStop : onGenerate} disabled={!isGenerating && (noPresets || !draft.prompt.trim() || isUploading)}>{isGenerating ? <><Square size={15} fill="currentColor" />停止生成</> : isUploading ? <><LoaderCircle size={16} className="animate-spin" />上传中…</> : <><SendHorizontal size={16} />生成图片</>}</button>
  </section>;
}
